...
2. The land data at 10 crosscuts for each land in  (`ccdata`).  
3. The land data averaged across crosscuts (`ccdata_w_resid`). This is most likely what you want to start with for modeling.  
...
hamby44_eval <- prediction_scores(hamby44_eval, "logistic")
```
...
plot_scores <- function(dataset){
  plot_df <- hamby44_eval %>% ungroup() %>% 
    select(dplyr::contains("score")) %>% 
    gather(method, score) %>% 
...
plot_scores_zoom <- function(dataset, zoom_cutoff){
  plot_df <- hamby44_eval %>% ungroup() %>% 
    select(dplyr::contains("score")) %>% 
    gather(method, score) %>% 
    mutate(GrooveMethod = unlist(purrr::map(method, .f = function(x){
...
```{r plot_scores}
plot_scores(hamby44_eval)

plot_scores_zoom(hamby44_eval, 5000)
```
