...
cutoff_frame <- function(cutoff_vec, response_col, prob_col){
  fpr <- rep(NA, times = length(cutoff_vec))
  dat <- data.frame(prob_col, response_col)
  all_negs <- sum(dat$response_col == 0)
  for(i in 1:length(cutoff_vec)){
    dat$class_pred <- ifelse(prob_col <= cutoff_vec[i], 0, 1)
    fpr[i] <- sum(dat$class_pred == 1 & dat$response_col == 0)/all_negs
  }
  return(data.frame(cutoff_vec, fpr))
}


h44_cutoffs <- seq(0,1, length.out = 1000)
#h44_fprs <- cutoff_frame(h44_cutoffs, response_col = h44_results$truth_01, prob_col = h44_)
...
Rollapply & `r round(auc_roll, 3)` & .1 &  \\ \hline
LASSO basic & `r round(auc_lb, 3)` & .1 &  \\ \hline
LASSO full & `r round(auc_lf, 3)` & .1 &  \\ \hline
Bayesian Changepoint & `r round(auc_bcp, 3)` & .1 & \\ \hline
Manual ID & `r round(auc_manual, 3)` & .1 & \\ \hline 
\end{tabular}
\end{table}
...
\begin{tabular}{llll}
\textbf{Method} & \textbf{AUC} & \textbf{Controlled FP Rate} & \textbf{Cutoff} \\ \hline
Rollapply & `r round(auc_roll_p, 3)` &  & \\ \hline
LASSO basic & `r round(auc_lb_p, 3)` &  &  \\ \hline
LASSO full & `r round(auc_lf_p, 3)` &  &  \\ \hline
Bayesian Changepoint & `r round(auc_bcp_p, 3)` &  & \\ \hline
Manual ID & `r round(auc_manual_p, 3)` &  & \\ \hline 
\end{tabular}
\end{table}
...
\begin{tabular}{llll}
\textbf{Method} & \textbf{AUC} & \textbf{Controlled FP Rate} & \textbf{Cutoff} \\ \hline
Rollapply & `r round(auc_roll_h, 3)` &  & \\ \hline
LASSO basic & `r round(auc_lb_h, 3)` &  &  \\ \hline
LASSO full & `r round(auc_lf_h, 3)` &  &  \\ \hline
Bayesian Changepoint & `r round(auc_bcp_h, 3)` &  & \\ \hline
Manual ID & `r round(auc_manual_h, 3)` &  & \\ \hline 
\end{tabular} 
\end{table}
