...
LASSO basic & `r round(auc_lb_p, 3)` &  `r round(pho_lb_table$cutoff, 3)` &`r pho_lb_table$fn` & `r pho_lb_table$tp`&`r pho_lb_table$tn` & `r round(pho_lb_table$acc, 2)` & 6 min. \\ \hline
...
LASSO basic & `r round(auc_lb_h, 3)` &  `r round(hou_lb_table$cutoff, 3)` &`r hou_lb_table$fn` & `r hou_lb_table$tp`&`r hou_lb_table$tn` & `r round(hou_lb_table$acc, 2)` & 12 min. \\ \hline
...
The initial appeal of unsupervised methods such as Bayesian changepoint was the lack of dependence on training data and hence, potential generalizability advantages. This advantage did not bear out in the test sets presented here, and it appears the LASSO methods trained on Hamby set 44 generalized effectively to new data.  
