...
hamby44 <- hamby44 %>% mutate(ccdata_w_resid = purrr::map(ccdata_w_resid, .f = function(bullet){
  bullet$response <- ifelse(bullet$y <= left_groove | bullet$y >= right_groove, 1, 0)
  return(bullet)
}))


bullet <- hamby44$ccdata_w_resid[[1]]

lm0 <- lm(response~ rlo_resid + side + depth + side*depth, data = bullet)
bullet$lm0_pred <- predict(lm0, newdata = bullet)

bullet %>% ggplot() + geom_point(aes(x = y, y = value_std, colour = lm0_pred)) + theme_bw()
 ```
