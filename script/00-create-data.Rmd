...
plot_grooves_lassofull <- function(datafile, bullet_num){
  bullet <- datafile$ccdata[[bullet_num]]
  
  ggplot() + geom_point(aes(x = bullet$x, y = bullet$value)) + theme_bw() + 
    geom_vline(aes(xintercept = datafile$grooves_lassofull[[bullet_num]]$groove[1])) + 
    geom_vline(aes(xintercept = datafile$grooves_lassofull[[bullet_num]]$groove[2]))
}
...
```{r}
#saveRDS(houston-test_paper, file = "../data/houston-test/houston-test_paper.rda")
```