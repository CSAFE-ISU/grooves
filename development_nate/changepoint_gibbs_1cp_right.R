cp1_gibbs_right <- function(data, iter, start.vals, prop_var, cp_prop_var, tol = 10, warmup = 5000, verbose = FALSE)
{
  ...
  for(i in 1:(warmup))
  {
    ...
    if(prop <= tol + interval[1] || prop >= -tol + interval[2])
    {
      ...
      med2 <- median(data[data$x <= xrange[2,2] & data$x > xrange[2,1], ]$x)
      mu2 <- ((data[data$x <= xrange[2,2] & data$x > xrange[2,1], ]$x - med2) / (xrange[2,2] - xrange[1,1])) * beta[1]
      mu1 <- rep(0, times = length(temp_dat1))
      
      prop_med2 <- median(data[data$x > prop & data$x <= interval[2], ]$x)
      prop_mu2 <- ((data[data$x > prop & data$x <= interval[2], ]$x - prop_med2) / (xrange[2,2] - xrange[1,1])) * beta[1]
  ...
  prop_var[[2]] <- 2.4^2 * var(cbind(par$sigma[warmup/2:warmup,2], par$l[warmup/2:warmup,2], par$tau[warmup/2:warmup,2], par$beta[warmup/2:warmup,1])) / 4
