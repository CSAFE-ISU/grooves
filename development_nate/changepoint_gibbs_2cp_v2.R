...
cp2_gibbs_v2 <- function(data, iter, start.vals, prop_var, cp_prop_var, tol = 10, warmup = 5000, verbose = FALSE)
{
  ...
  for(i in 1:(warmup))
  {
    ...
    for(j in 1:3)
    {
      if(j == 1)
      {
        prop <- as.numeric(rmvnorm(n = 1, mean = c(sigma[j], l[j], tau[j], beta[1]), sigma = prop_var[[j]]))
      }
      if(j == 3)
      {
        prop <- as.numeric(rmvnorm(n = 1, mean = c(sigma[j], l[j], tau[j], beta[2]), sigma = prop_var[[j]]))
      }
      if(j == 2)
      {
      ...
      if(j == 3)
      {
        ...
        if(j == 1)
        {
          ...
          mu <- ((data[data$x <= xrange[j,2] & data$x > xrange[j,1], ]$x - med)/(xrange[3,2] - xrange[1,1])) * beta[1]
          prop_mu <- ((data[data$x <= xrange[j,2] & data$x > xrange[j,1], ]$x) / (xrange[3,2] - xrange[1,1])) * prop[4]
        ...
        {
          med <- median(data[data$x <= xrange[j,2] & data$x > xrange[j,1], ]$x)
          mu <- ((data[data$x <= xrange[j,2] & data$x > xrange[j,1], ]$x - med)/(xrange[3,2] - xrange[1,1])) * beta[2]
          prop_mu <- ((data[data$x <= xrange[j,2] & data$x > xrange[j,1], ]$x) / (xrange[3,2] - xrange[1,1])) * prop[4]
    ...
    if(prop[1] >= prop[2] - tol || prop[1] <= tol + interval[1] || prop[2] >= -tol + interval[2])
    {
      ...
      med3 <- median(data[data$x <= xrange[3,2] & data$x > xrange[3,1], ]$x)
      mu3 <- ((data[data$x <= xrange[3,2] & data$x > xrange[3,1], ]$x - med3) / (xrange[3,2] - xrange[1,1])) * beta[2]
      mu1 <- ((data[data$x <= xrange[1,2] & data$x > xrange[1,1], ]$x - med1) / (xrange[3,2] - xrange[1,1])) * beta[1]
      mu2 <- rep(0, times = length(temp_dat2))
      
      prop_med1 <- median(data[data$x <= prop[1] & data$x > interval[1], ]$x)
      prop_med3 <- median(data[data$x <= interval[2] & data$x > prop[2], ]$x)
      prop_mu3 <- ((data[data$x <= interval[2] & data$x > prop[2], ]$x - prop_med3) / (xrange[3,2] - xrange[1,1])) * beta[2]
      prop_mu1 <- ((data[data$x <= prop[1] & data$x > interval[1], ]$x - prop_med1) / (xrange[3,2] - xrange[1,1])) * beta[1]
  ...
  prop_var[[1]] <- 2.4^2 * var(cbind(par$sigma[round(warmup/2):warmup,1], par$l[round(warmup/2):warmup,1], par$tau[round(warmup/2):warmup,1], par$beta[round(warmup/2):warmup,1])) / 3
  prop_var[[2]] <- 2.4^2 * var(cbind(par$sigma[round(warmup/2),2], par$l[round(warmup/2):warmup,2], par$tau[round(warmup/2):warmup,2])) / 3
  prop_var[[3]] <- 2.4^2 * var(cbind(par$sigma[round(warmup/2):warmup,3], par$l[round(warmup/2):warmup,3], par$tau[round(warmup/2):warmup,3], par$beta[round(warmup/2):warmup,2])) / 3
  cp_prop_var <- 2.4^2 * var(par$cp[round(warmup/2):warmup,]) / 2
