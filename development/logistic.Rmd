...
hamby44 <- hamby44 %>% mutate(operator = purrr::map_chr(source, .f = function(bullet){
  split <- unlist(strsplit(bullet, " - "))
  namedat <- split[length(split)]
  operator <- gsub(".{4}$", "", namedat)
  return(operator)
}))

hamby44_long <- hamby44 %>% unnest(ccdata_w_resid)
head(hamby44_long)
```
