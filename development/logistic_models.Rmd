...
cutoff_df <- cutoff_df %>% mutate(error_diff = abs(sensitivity - specificity))
...
cutoff_df %>% ggplot() + theme_bw() + geom_line(aes(x = cutoff_vals, y = sensitivity), colour = "green") + geom_line(aes(x = cutoff_vals, y = specificity))
cutoff_df <- cutoff_df %>% mutate(error_diff = abs(sensitivity - specificity))
which.min(cutoff_df$error_diff)
cutoff_df[7,]
